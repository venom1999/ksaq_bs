...
export default class Examination extends React.Component{
    ...
    static navigationOptions = ({navigation}) => {
        ...
        return ({
            title: params ? params.selectedExamName : '考试',
            headerStyle: {
    ...
    onBackButtonPressAndroid = () => {
        ...
        //     ToastAndroid.show('再按一次返回将退出考试', ToastAndroid.SHORT);
        //     return true;
        // }
        // Alert.alert(
        //     "提示",
    ...
    getExamData(){
        const {navigation} = this.props;
        let test_id = navigation.getParam('testID');
        this.setState({
            test_id:test_id
        })
        this.setState({
               //单个单选题分
               singleChooseScore: navigation.getParam('singleChooseScore'),
               //单个多选题分
               singleMultipleChooseScore: navigation.getParam('singleMultipleChooseScore'),
               //单个判断题分
               singleJudgeScore: navigation.getParam('singleJudgeScore'),
               examTime: navigation.getParam('examTime'),
               totalTime: navigation.getParam('totalTime'),
               ExamArray: navigation.getParam('ExamArray'),
               questionNum: navigation.getParam('questionNum'),
               loaded: true,
               hasPaper: true,
               commitArray: navigation.getParam('commitArray'),
               markArray: navigation.getParam('markArray'),
               testType: navigation.getParam('testType'),
               edu_category_name:navigation.getParam('edu_category_name'),
        })
    }
    toQuestion(index) {
    ...
    ToFullScreen(url){
        console.log("到这里了")
        this.props.navigation.push('ExamfullScreenVideo',{
            someuri: url
        })
    }
    ...
    storeError(){
        ...
        const chooseScore = this.state.singleChooseScore;
        const multipleChooseScore = this.state.singleMultipleChooseScore;
        const judgeScore = this.state.singleJudgeScore;
        ...
        for (let i = 0; i < examArray.length; i++) {
            ...
            if (!(this.state.commitArray[i] == null || this.state.commitArray[i] === '') && this.state.commitArray[i] !== examArray[i].answer) {
                let wrongQuestion = {edu_category_id: 1,
                    test_name: navigation.getParam('selectedExamName'),
                    ...
                    edu_category_name: this.state.edu_category_name};
        ...
        formData.append('testID',AES.Encrypt(this.state.test_id));
        formData.append('userID',AES.Encrypt(this.state.userID));
        formData.append('score',AES.Encrypt(totalScore));
        formData.append('testType',AES.Encrypt(this.state.testType));
        HttpUtils.PostFetch(URL.url + "/OnlineExam/submitScore",formData)
            .then(res => {
                let returntext = res.returntext;
                console.log("res.returntext----------------"+res.returntext);
                if('success'===returntext){
                    Alert.alert(
                        '考试结果',
